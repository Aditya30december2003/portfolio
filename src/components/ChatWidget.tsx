...
const FAQ: Record<string, string> = {
  ...
  'How can I contact you?': 'You can contact me using the form or via my email at adityasmjain@gmail.com.',
}
...
export default function ChatWidget() {
  ...
  const sendMessage = async () => {
    ...
    // functional update keeps types tight
    ...
    if (remaining <= 0) {
      const answer = FAQ[userInput] || "I'm out of answers today. Try tomorrow or check the FAQ section."
      setMessages((prev) => [...prev, { sender: 'bot' as const, text: answer }])
      setIsTyping(false)
      return
    }
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { sender: 'bot' as const, text: data.reply || 'Something went wrong.' },
      ])
    } catch {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans">
      {/* Chat Panel */}
      <div
        className={`w-80 sm:w-96 h-[450px] bg-white rounded-lg shadow-2xl border border-gray-200 
